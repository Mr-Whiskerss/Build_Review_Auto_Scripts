...
function Write-Summary {
    ...
    [void]$Script:OutputBuffer.AppendLine(('Run As Administrator: {0}' -f $Script:IsAdmin))
...
function Save-Report {
    try {
        ...
        Write-Host ('[-] Failed to save report: {0}' -f $_) -ForegroundColor Red
    }
    ...
    if ($HTMLReport) {
        ...
        try {
            ...
            $findingsHtml = ($Script:Findings | Where-Object { $_.Severity -in @('Critical', 'High') } | ForEach-Object {
                '<li class="{0}">[{1}] {2}</li>' -f $_.Severity.ToLower(), $_.Severity.ToUpper(), $_.Title
            }) -join "`n    "

            $reportBody = [System.Web.HttpUtility]::HtmlEncode($Script:OutputBuffer.ToString())

            $sb = New-Object System.Text.StringBuilder
            [void]$sb.AppendLine('<!DOCTYPE html>')
            [void]$sb.AppendLine('<html>')
            [void]$sb.AppendLine('<head>')
            [void]$sb.AppendLine(('    <title>Windows Build Review - {0}</title>' -f $env:COMPUTERNAME))
            [void]$sb.AppendLine('    <style>')
            [void]$sb.AppendLine('        body { font-family: Segoe UI, Arial, sans-serif; margin: 40px; background: #1a1a2e; color: #eee; }')
            [void]$sb.AppendLine('        h1 { color: #00d4ff; border-bottom: 2px solid #00d4ff; padding-bottom: 10px; }')
            [void]$sb.AppendLine('        h2 { color: #00d4ff; margin-top: 30px; }')
            [void]$sb.AppendLine('        .critical { color: #ff4757; font-weight: bold; }')
            [void]$sb.AppendLine('        .high { color: #ffa502; font-weight: bold; }')
            [void]$sb.AppendLine('        .medium { color: #ffda79; }')
            [void]$sb.AppendLine('        .low { color: #7bed9f; }')
            [void]$sb.AppendLine('        .pass { color: #2ed573; }')
            [void]$sb.AppendLine('        .info { color: #70a1ff; }')
            [void]$sb.AppendLine('        pre { background: #16213e; padding: 15px; border-radius: 5px; overflow-x: auto; }')
            [void]$sb.AppendLine('        .summary { background: #16213e; padding: 20px; border-radius: 10px; margin: 20px 0; }')
            [void]$sb.AppendLine('        .summary-item { display: inline-block; margin: 10px 20px; text-align: center; }')
            [void]$sb.AppendLine('        .summary-count { font-size: 36px; font-weight: bold; }')
            [void]$sb.AppendLine('    </style>')
            [void]$sb.AppendLine('</head>')
            [void]$sb.AppendLine('<body>')
            [void]$sb.AppendLine('    <h1>Windows Build Review Report</h1>')
            [void]$sb.AppendLine(('    <p><strong>Host:</strong> {0}</p>' -f $env:COMPUTERNAME))
            [void]$sb.AppendLine(('    <p><strong>Date:</strong> {0}</p>' -f (Get-Date)))
            [void]$sb.AppendLine(('    <p><strong>Assessed By:</strong> {0}</p>' -f $env:USERNAME))
            [void]$sb.AppendLine('    <div class="summary">')
            [void]$sb.AppendLine('        <h2>Findings Summary</h2>')
            [void]$sb.AppendLine(('        <div class="summary-item"><div class="summary-count critical">{0}</div>Critical</div>' -f $critCount))
            [void]$sb.AppendLine(('        <div class="summary-item"><div class="summary-count high">{0}</div>High</div>' -f $highCount))
            [void]$sb.AppendLine(('        <div class="summary-item"><div class="summary-count medium">{0}</div>Medium</div>' -f $medCount))
            [void]$sb.AppendLine(('        <div class="summary-item"><div class="summary-count low">{0}</div>Low</div>' -f $lowCount))
            [void]$sb.AppendLine(('        <div class="summary-item"><div class="summary-count pass">{0}</div>Pass</div>' -f $passCount))
            [void]$sb.AppendLine('    </div>')
            [void]$sb.AppendLine('    <h2>Critical &amp; High Findings</h2>')
            [void]$sb.AppendLine('    <ul>')
            [void]$sb.AppendLine(('    {0}' -f $findingsHtml))
            [void]$sb.AppendLine('    </ul>')
            [void]$sb.AppendLine('    <h2>Full Report</h2>')
            [void]$sb.AppendLine(('    <pre>{0}</pre>' -f $reportBody))
            [void]$sb.AppendLine('</body>')
            [void]$sb.AppendLine('</html>')
            ...
            Write-Host ('[-] Failed to save HTML report: {0}' -f $_) -ForegroundColor Red
        }
    }
}
...
function Main {
    ...
    Write-Host ('|                           Version {0}                                   |' -f $Script:Version) -ForegroundColor Cyan
    Write-Host "+==============================================================================+" -ForegroundColor Cyan
    Write-Host ""
    Write-Host ('[*] Target Host: {0}' -f $env:COMPUTERNAME) -ForegroundColor Yellow
    Write-Host ('[*] Assessment Date: {0}' -f (Get-Date)) -ForegroundColor Yellow
    Write-Host ('[*] Running as Administrator: {0}' -f $Script:IsAdmin) -ForegroundColor Yellow
    Write-Host ('[*] Output File: {0}' -f $OutputPath) -ForegroundColor Yellow
    Write-Host ""
    ...
    [void]$Script:OutputBuffer.AppendLine("===========================")
    [void]$Script:OutputBuffer.AppendLine(('Generated: {0}' -f (Get-Date)))
    [void]$Script:OutputBuffer.AppendLine(('Host: {0}' -f $env:COMPUTERNAME))
    [void]$Script:OutputBuffer.AppendLine(('User: {0}' -f $env:USERNAME))
    [void]$Script:OutputBuffer.AppendLine(('Administrator: {0}' -f $Script:IsAdmin))
    [void]$Script:OutputBuffer.AppendLine("")
